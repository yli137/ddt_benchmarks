                             ...
                             int num, int *disp, int *len, int *dst_disp )
{
    ...
    for( t = 0; t < trials; t++ ) {
        ...
        for( c = 0; c < cycles; c++ ) {
            ...
            while( current_pos < incount - pipe_size ){

                for( m = 0; m < pipe_size; m++, i++ )
                    __builtin_prefetch( inbuf + i * extent );
                for( k = 0; k < num; k++ ){
                    keep_pos = current_pos;
                    for( j = 0 ; j < pipe_size && current_pos < incount; j++, current_pos++ ){
                        memcpy( outbuf + current_pos * ddt_size + dst_disp[k],
                                inbuf + current_pos * extent + disp[k],
                                len[k] );
                    }
                    current_pos = keep_pos;
                }
                current_pos += pipe_size;
